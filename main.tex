...
\begin{block}{API Server}
C'est le point d'entrée pour toutes les commandes utilisé pour contrôler le cluster.\\
Il récupère les commandes (REST), les valides, et les exécutes.  \\Le résultat de ces commandes est conservé dans \textit{etcd}.
\end{block}

\begin{block}{etcd}
etcd est utilisé pour partager la configuration et la découverte de service.\\ Il fournit une API pour des opérations CRUD et permet aux nœuds de s'enregistrer (pour pouvoir être informé d'un changement de configuration)\\
\vspace{10px}
...
\begin{block}{kube-controller-manager}
